...
if (isset($_POST['editBtn'])) {
    if (isset($_POST['Size'])) {
        ...
        $sql = "UPDATE cart SET Quantity = ?, ProductSize = ? WHERE ID = ? AND CustomerID = ?";
        $stmt = $con->prepare($sql);
        $stmt->bind_param('isis', $quantity, $size, $ID, $cID);
        if ($stmt->execute()) {
            (isset($_GET['user_id'])) ?
                            $cID = (trim($_GET['user_id'])) :
                            $cID = "";
        ...
        $message['failed'] = 'No size was detected! Edit is cancelled.';
    }
}
?>
    ...
    <?php
        ...
        $result = $con->query($sql);
            ...
            $user_id = trim($_GET['user_id']):
                $user_id = "";
                //record not found
                echo "<div class='error'>There's nothing inside your cart [ <a href='homepage.php?user_id=$user_id'>Back to home page</a> ]</div>";
                $hideCart = true;
            }
            $con->close();
            $result->free();
...
<div>
       ...
       ", $record->CartID, $record->ProductImage, $record->ProductName, $record->Price, $record->Quantity, $record->ID, $record->CustomerID);
                        ...
                        $result->free();
                        ?>
                        <input type="submit" class="deleteBtn2" value="Delete Selected Items" name="deleteBtn2" />
                ...
                (isset($_GET['user_id'])) ?
                                $user_id = trim($_GET['user_id']) :
                                $user_id = "";
                printf("<form action='payment.php?user_id=%s' method='POST'>", $user_id);
                ?>
